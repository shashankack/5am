 ...
 */
export const INTRO_BOOT_SCRIPT = `(function(){try{var k=${JSON.stringify(INTRO_PLAYED_KEY)};if(sessionStorage.getItem(k)==="1"||window.matchMedia("(prefers-reduced-motion: reduce)").matches)return;var s=document.createElement("style");s.id=${JSON.stringify(INTRO_BOOT_STYLE_ID)};s.textContent=${JSON.stringify(`#${NAVBAR_ID}{transform:translate3d(0,-100%,0)}`)};document.head.appendChild(s);}catch(e){}})();`;
